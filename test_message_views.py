...
#to run: FLASK_ENV=production python3 -m unittest test_message_views.py
...
# started at ===========================================started at ===============================started at==========================started at===============

    def test_add_no_session(self):
        # you have to mimic the session which is why you do this 
        with self.client as c:
            # mimicing a post
            resp = c.post("/messages/new", data={"text": "Hello"}, follow_redirects=True)
            # checking to see if the page is valid
            self.assertEqual(resp.status_code, 200)
            # checking to see if a certain route is limited to only a user logging in
            self.assertIn("Access unauthorized", str(resp.data))

    def test_add_invalid_user(self):
        with self.client as c:
            with c.session_transaction() as sess:
                #  putting in a user that does not exist to see if the page will restrict a client
